...
| G | Import-contract edge attribution | Superseded by Plan 2: the validator no longer generates one coarse `layers` contract per project. It now generates a per-module `layers` contract (ARCH-001/002/005, scoped to that module's own domain/application/infrastructure) plus separate per-context `forbidden` contracts for ARCH-006 (entrypoints), ARCH-046 (cross-module isolation) and ARCH-052 (read/write isolation) — each rule already maps to its own contract, so a broken contract attributes to exactly the rule(s) it covers rather than four at once. The remaining coarse-signal gap — a rule whose contract is genuinely never emitted for a given project shape — is no longer a vacuous PASS: I3 (Plan 2 fix wave) makes `ImportContractsCheck` report `SKIP` for any rule_id not in import-linter's `covered` set, closing the vacuous-PASS half of the limitation row K used to track. |
...
| K | ~~Import-contract coarse-signal limitations beyond edge attribution~~ **CLOSED** | Was: ARCH-012/034/035 (and, post-Plan-2, ARCH-006/046/052) vacuously PASS when their contract isn't emitted at all. Closed by Plan 2's I3 fix: `ImportContractsCheck` now reports `SKIP` for any rule_id not in import-linter's `covered` output instead of routing it through `outcome_for` as an unconditional PASS — "nothing to check" is no longer indistinguishable from "checked, no violation found". The other half of this row, `_STATUS_RE` not stripping ANSI color codes, remains an accepted v1 limitation (import-linter is invoked with `--no-cache` and no explicit color flag, and the fixtures/CI environment does not colorize; revisit if that changes). |
| L | ~~Validator must learn the aggregate-module level~~ **CLOSED** | Shipped by Plan 2 ("Aggregate Modules, Read Layer, Context Graph"): `ProjectLayout` and every check now operate on `<context>/<module>/{domain,application,infrastructure}`, with `<context>/shared/`, `<context>/read/`, and `<context>/entrypoints/` as context-level siblings. |
...
| P | `rules/*.yaml` examples still use the pre-aggregate-module flat shape | ~19 of 53 rules' `correct`/`incorrect` fields (mostly `dependencies.yaml`) still show `sales/domain/...` instead of `sales/orders/domain/...` — inherited from Plan 1, not worsened by Plan 2's fix wave. **Deliberately deferred**: no code reads these fields and `docs --check` is unaffected, and the fix is mechanical-but-pervasive — best done as one dedicated sweep with a regression test, not folded piecemeal into unrelated fix waves. **This row blocks starting the template/skill/architecture-reviewer work (Plan 3+)**, since that work consumes `rules/*.yaml` examples directly as scaffolding/review input — an agent following ARCH-001's current `correct` example today would produce a layout ARCH-048 rejects. No regression test exists yet for this (one asserting no flat-path examples remain would fail immediately against today's content); it lands together with the actual cleanup. |
...
| R | ARCH-008 and ARCH-033 are `tier: core` with no automated check | Both rules are catalogued `tier: core` (`rules/dependencies.yaml`, `rules/model_integrity.yaml`) but no check implements either — they never appear in `Report.collect`'s output. Plan 2's I1 fix makes this visible rather than hidden: `--core` now backfills a synthetic `SKIP` row for each so the reported count always matches the header's "(12)" claim, instead of silently showing 10. I1 does not implement the checks themselves. v1.1: implement ARCH-008 and ARCH-033. |
