...
| H | ~~`_PACKAGED_RULES` path resolution (`cli.py`)~~ RESOLVED | `rules/*.yaml` and `rules/.released/` were moved (`git mv`, history preserved) into `src/arch_standard/rules/_catalog/`, and a single `importlib.resources`-based `packaged_rules_dir()` helper in `catalog.py` replaces `_PACKAGED_RULES` everywhere it was used (`cli.py`, `docgen.py`). A source checkout and an installed wheel now resolve the catalog through the exact same code path, with no dev-checkout fallback branch, and a built wheel now ships all 53 rules. Resolved by 77e82f5. |
...
| J | ~~CI wiring for waivers and `arch-standard check`~~ PARTIALLY RESOLVED | The self-validation half is closed: CI's `validate` job now runs `arch-standard check` against `tests/fixtures/good_project` and `modular_project` (must pass) and `bad_project` (must fail, asserted via an if/exit construct that `set -e` cannot defeat), plus `arch-standard release-check` on this repo's own catalog on every push and PR; a separate `e2e` job runs `make e2e`; `make selfcheck` mirrors the same steps locally. Resolved by abdd914. The other half of this row -- CI printing the active-waiver count per rule (spec §13) -- was not one of this plan's nine acceptance-audit blockers and remains open; still v1.1 follow-up. |
